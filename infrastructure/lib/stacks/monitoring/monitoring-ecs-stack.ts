  ...
  private createEcsCluster(
    ...
    const efsCommands = fileSystem
      ? [
          ...
          "mkdir -p /mnt/efs",
          "",
          "echo 'Resolving EFS mount target IP...'",
          `EFS_IP=$(nslookup ${fileSystem.fileSystemId}.efs.${cdk.Stack.of(this).region}.amazonaws.com | grep 'Address:' | tail -n1 | awk '{print $2}')`,
          'echo "EFS IP: $EFS_IP"',
          "",
          "# Mount EFS using NFS4 (fallback to IP if DNS fails)",
          'if [ -n "$EFS_IP" ]; then',
          '  echo "Mounting EFS using IP: $EFS_IP"',
          "  mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 $EFS_IP:/ /mnt/efs",
          "else",
          '  echo "DNS resolution failed, trying EFS helper with TLS"',
          ...
          "",
          "echo 'Removing any existing directories/symlinks...'",
          "rm -rf /mnt/prometheus-data /mnt/grafana-data",
          "",
          "echo 'Creating symlinks to EFS...'",
          ...
          "",
          "echo 'Creating config directories (local, not on EFS)...'",
    ...
    autoScalingGroup.addUserData(
      ...
      "exec > >(tee -a /var/log/monitoring-setup.log)",
      "exec 2>&1",
      "",
      "echo '========================================='",
      ...
      "date",
      "",
      "# Stop ECS agent to prevent tasks from starting before setup completes",
      "echo 'Stopping ECS agent...'",
      "systemctl stop ecs || true",
      "",
      "# Add ec2-user to docker group for docker command access",
      ...
      "EOF",
      "",
      "# Set permissions on config directories (local, not on EFS)",
      ...
      "",
      "# Set permissions on EFS data directories (if using EFS)",
      "if [ -d /mnt/efs ]; then",
      "  echo 'Setting permissions on EFS directories...'",
      ...
      "ls -la /mnt/prometheus-config/prometheus.yml",
      "",
      "echo 'Grafana datasource:'",
      "ls -la /mnt/grafana-provisioning/datasources/prometheus.yml",
      "",
      "echo 'Grafana dashboards:'",
      "ls -la /mnt/grafana-provisioning/dashboards/dashboards.yml",
      "",
      ...
      "date",
      "",
      "# Start ECS agent now that setup is complete",
      "echo 'Starting ECS agent...'",
      "systemctl start ecs",
      "systemctl enable ecs",
      "",
      "echo '✓ ECS agent started - ready for tasks'",
